...
const authStore = useAuthStore(); //Initialize auth store instance
...
onMounted(async () => {
  try {
    // Add the headers configuration object here as the second parameter
    const response = await fetch(`${API_BASE}/api/events`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "Accept": "application/json",
        "Authorization": `Bearer ${authStore.token}`, // Passes the token securely
      },
    });
...
const filteredAndSortedEvents = computed(() => {
  ...
  if (selectedCategories.value.length > 0) {
    result = result.filter((event) => {
      ...
      const eventTags = event.category_tags
        .split(",")
        .map((t) => t.trim().toLowerCase());
      return selectedCategories.value.some((cat) =>
        eventTags.includes(cat.toLowerCase()),
      );
    });
  }
  ...
  <div class="min-h-screen p-4 sm:p-8 max-w-7xl mx-auto">
    ...
    <div class="space-y-6">
      ...
      <div
        ...
        class="text-red-500 bg-red-50 dark:bg-red-950/20 p-4 rounded-xl text-center"
      >
        {{ errorMessage }}
      </div>
      ...
      >
        ...
        <p class="text-sm text-gray-400 mt-1">
          Try adjusting your search criteria or toggling different filter
          categories.
        </p>
      </div>
