  ...
  <div class="min-h-screen bg-white dark:bg-gray-900 pb-16">
    <div class="px-4 sm:px-6 lg:px-8 py-8 max-w-7xl mx-auto">
      <div class="flex flex-col gap-3 mb-8">
        <div>
          <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Scan QR Code</h1>
          <p class="text-gray-600 dark:text-gray-400 mt-2">
            Use the camera to scan a student ticket and mark attendance.
          </p>
        </div>
        ...
        <div class="rounded-3xl bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 p-4">
          ...
          <div class="flex items-center gap-3 flex-col sm:flex-row">
            ...
            <button
              type="button"
              class="rounded-2xl bg-blue-600 text-white px-5 py-3 font-semibold transition hover:bg-blue-500 disabled:opacity-50"
          ...
          <p class="mt-3 text-sm text-gray-500 dark:text-gray-400">
            If the camera prompt does not appear, make sure your browser has camera access enabled.
          </p>
        </div>
      </div>
          ...
          <div class="space-y-4">
            <div class="rounded-3xl bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-800 p-6 shadow-sm">
              <div class="flex items-center justify-between">
                ...
                <span
                  :class="['inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold', scanning ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-200' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300']"
                >
              ...
              <div class="mt-5 space-y-3">
                <div class="rounded-3xl bg-gray-50 dark:bg-gray-900 p-4">
                  ...
                  <p class="mt-2 text-sm text-gray-800 dark:text-gray-100 break-words">{{ lastScanText || 'No scans yet' }}</p>
                </div>

                <div class="rounded-3xl p-4" :class="messageType === 'success' ? 'bg-emerald-50 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200' : 'bg-rose-50 text-rose-800 dark:bg-rose-900/40 dark:text-rose-200'">
                  <p class="text-sm font-semibold">{{ messageTitle }}</p>
                  <p class="mt-2 text-sm leading-relaxed">{{ statusMessage || messageFallback }}</p>
                </div>
              </div>
            </div>
...
const messageFallback = computed(() => {
  if (messageType.value === 'success') return 'The ticket was accepted and attendance has been recorded.';
  if (messageType.value === 'error') return 'Please review the scanned code or try again.';
...
const loadCameras = async () => {
  try {
    ...
    statusMessage.value = 'Unable to access camera devices. Please allow camera permissions.';
    messageType.value = 'error';
  }
};
...
const handleScan = async (decodedText) => {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/society/events/${selectedEventId.value}/checkin`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok && data.status === 'success') {
      ...
      statusMessage.value = data.message || 'Check-in failed. Please try again.';
      messageType.value = 'error';
    }
  } catch (error) {
    statusMessage.value = error.message || 'Unable to complete attendance check-in.';
    messageType.value = 'error';
  } finally {
