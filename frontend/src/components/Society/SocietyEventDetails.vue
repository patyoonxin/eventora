...
onMounted(async () => {
  try {
    ...
    const response = await fetch(`http://localhost:8000/api/society/upcoming-events`, {
      headers: {
...
const handleEditDetails = () => {
  alert('Redirecting to Edit Form layout...')
}
...
<template>
  <div class="min-h-screen bg-white dark:bg-gray-900 pb-24">
    ...
    <div v-if="isLoading" class="text-center py-20 text-gray-500">Loading organizer metrics...</div>
    ...
    <div v-else class="max-w-xl mx-auto px-4 pt-4 flex flex-col min-h-[90vh] relative">
      ...
      <div class="relative w-full h-64 sm:h-72 bg-purple-200 dark:bg-purple-900 rounded-[2.5rem] overflow-hidden shadow-sm mb-6">
        ...
        />
        <button @click="router.back()" class="absolute top-5 left-5 w-12 h-12 flex items-center justify-center bg-white dark:bg-gray-800 text-gray-800 dark:text-white rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all">
      ...
      <div class="mt-8 bg-gray-50 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-800 rounded-[2rem] py-8 px-6 text-center">
        <div class="flex flex-col items-center justify-center space-y-2">
          ...
          <a 
            href="#" 
            @click.prevent 
            class="text-sm font-medium text-gray-400 dark:text-gray-500 hover:underline transition-all"
          >
            {{ event.supporting_document || 'Supporting Document.pdf' }}
          </a>
        </div>
      </div>
      ...
      <div class="mt-6 text-left px-2 space-y-1">
        <div class="text-sm text-gray-400 dark:text-gray-500 font-medium">
          Status: <span class="font-bold text-gray-600 dark:text-gray-300 capitalize">{{ event.status || 'Approved' }}</span>
        </div>
        <div>
          <button 
      ...
      <div class="mt-auto pt-8 pb-6 flex items-center gap-4 bg-white dark:bg-gray-900">
        ...
        <div class="flex items-center justify-center gap-2 px-5 py-4 bg-purple-100 dark:bg-purple-950/60 rounded-2xl min-w-[110px] sm:min-w-[130px]">
          <span class="text-base text-gray-700 dark:text-gray-300 pi pi-users"></span>
          <span class="text-base sm:text-lg font-bold text-gray-800 dark:text-gray-200">
